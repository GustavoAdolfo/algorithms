...
def check_keydown_events(event, ai_settings, screen, ship, bullets):
    """ Responde a presionamentos de teclas """
...
def check_play_button(ai_settings, screen, stats, sb, play_button, ship, aliens, bullets, mouse_x, mouse_y):
    ...
    if button_clicked and not stats.game_active:
        # Reinicializa as configurações do jogo
...
def update_screen(ai_settings, screen, stats, sb, ship, aliens, bullets, play_button):
    ...
    # Redesenha todos os projéteis atás da espaçonave e dos alienígenas
...
def update_bullets(ai_settings, screen, stats, sb, ship, aliens, bullets):
    ...
    # Livra-se dos projéteis que já desapareceram
...
def change_fleet_direction(ai_settings, aliens):
    """ Faz toda a frota decer e muda a sua direção """
    for alien in aliens.sprites():
...
def check_bullet_alien_collisions(ai_settings, screen, stats, sb, ship, aliens, bullets):
    ...
    if collisions:
        for alliens_collisions in collisions.values():
            stats.score += ai_settings.alien_points * len(alliens_collisions)
            sb.prep_score()
